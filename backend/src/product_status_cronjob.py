...
from src.database_models import Config, Product, ProductHist
...
async def fetch_and_store_product_status():
    ...
    with Session(engine) as session:
        ...
        is_price_drop_alert_config = session.exec(
            select(Config).where(Config.key == "is_price_drop_alert")).first()
        is_stock_change_alert_config = session.exec(
            select(Config).where(Config.key == "is_stock_change_alert")).first()
        ...
        is_price_drop_alert = is_price_drop_alert_config.value.lower(
        ) == "true" if is_price_drop_alert_config else False
        is_stock_change_alert = is_stock_change_alert_config.value.lower(
        ) == "true" if is_stock_change_alert_config else False
        ...
        if telegram_enabled:
            ...
            logger.info(
                f"Price drop alerts: {is_price_drop_alert}, Stock change alerts: {is_stock_change_alert}")
        else:
            logger.info(
        ...
        for product in products:
            try:
                logger.info(
                    f"Fetching product status for product: {product.name} (ID: {product.id})")
                ...
                if telegram_enabled and is_price_drop_alert and last_product_hist:
                    if product_status.price < last_product_hist.price:
                        ...
                        try:
                            ...
                            logger.info(
                                f"Price drop alert sent for {product.name}")
                ...
                logger.error(
                    f"Error processing product {product.name} (ID: {product.id}): {str(e)}")
                error_count += 1
                # Continue with next product even if one fails
                continue

        logger.info(
            f"Product status process completed. Success: {success_count}, Errors: {error_count}")
...
def should_run_analysis() -> bool:
    ...
    with Session(engine) as session:
        ...
        analysis_hour_config = session.exec(
            select(Config).where(Config.key == "analysis_hour")
        ).first()

        if not analysis_hour_config:
            logger.warning(
                "analysis_hour not found in Config. Using default value 12.")
            configured_hour = 12
        else:
            configured_hour = int(analysis_hour_config.value)
...
async def main():
    ...
    if should_run_analysis():
        ...
        logger.info(
            "Current hour does not match configured analysis hour. Skipping product status fetch.")
